...
#
#   npm run dev:restart:cloud:pg                 # LAN (phones on your WiFi can pair)
#   sh scripts/restart-dev-cloud-pg.sh --local  # localhost-only
#
...
case "$PG_URL" in
  *127.0.0.1:5433*|*localhost:5433*)
    if ! command -v docker >/dev/null 2>&1; then
      echo "restart-dev-cloud-pg: docker not found. Install docker, or export RELAY_BROKER_PUBLIC_POSTGRES_URL to your own Postgres." >&2
      exit 1
    fi
    if ! docker info >/dev/null 2>&1; then
      echo "restart-dev-cloud-pg: docker daemon not running. Start Docker Desktop first." >&2
      exit 1
    fi
    if docker ps --format '{{.Names}}' | grep -q '^sealwire-pg$'; then
      : # already running
    elif docker ps -a --format '{{.Names}}' | grep -q '^sealwire-pg$'; then
      echo "restart-dev-cloud-pg: starting existing sealwire-pg container"
      docker start sealwire-pg >/dev/null
    else
      echo "restart-dev-cloud-pg: creating sealwire-pg container on :5433"
    ...
    printf 'restart-dev-cloud-pg: waiting for postgres'
    i=0
    while [ "$i" -lt 30 ]; do
...
echo "restart-dev-cloud-pg: PUBLIC mode + Postgres, real provider, network=$net"
echo "restart-dev-cloud-pg: postgres=$PG_URL"
